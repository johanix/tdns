...
> **Partial.** Only the `parentsync:` block is documented here in full.
...
([special-features 1.1](special-features.md#11-parent-publishing-dsync)).
...
under `childsync.policies:` makes the daemon refuse to start, and a zone whose
...
| Key | Meaning |
|-----|---------|
| `parentsync.schemes` | which transports to try: `notify`, `update`, `api`. **Empty means nothing is ever forwarded** |
| `parentsync.update.keygen.algorithm` | algorithm for the SIG(0) keypair the agent signs UPDATEs with |
| `parentsync.update.bootstrap.methods` | SIG(0) bootstrap methods this agent will let a parent use; intersected with the parent's advertisement. Omitted means `[ at-apex, at-ns ]` |
| `parentsync.update.allow-insecure` | act on parent-derived input that cannot be authenticated (an unsigned KeyState response, an unvalidated SVCB advertisement). **Lab only.** Never waives a *bogus* DNSSEC verdict |
| `parentsync.api.*` | credentials for the HTTPS API scheme, one per parent |
...
`{ZONENAME}` substitution and the `childsync.*` subtree are documented with the
