...
algorithm (it will fail later, loudly, if the zone tries to sign with it). The
public half *is* parsed, as a cheap crypto-free consistency check: owner name
must match the zone, flags and keytag must match what the manifest claims. That
catches a manifest that has drifted from its own files.
