...
func (kdb *KeyDB) ChildSig0Mgmt(kp tdns.KeystorePost) (tdns.KeystoreResponse, error) {

	const (
		addkeysql = `
INSERT OR REPLACE INTO ChildSig0Keys (child, keyid, trusted, keyrr) VALUES (?, ?, ?, ?)`
		getallchildsig0keyssql = `
SELECT child, keyid, trusted, validated, keyrr FROM ChildSig0Keys`
		getonechildsig0keyssql = `
SELECT child, keyid, trusted, validated, keyrr FROM ChildSig0Keys WHERE owner=? AND keyid=?`
		childsig0keyupdatetrustsql = `
UPDATE ChildSig0Keys SET trusted=? WHERE child=? AND keyid=?`
	)
