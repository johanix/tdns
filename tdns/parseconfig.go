...
func (conf *Config) ParseZones(reload bool) ([]string, error) {
	...
	if Globals.App.Type == AppTypeAgent && len(primary_zones) > 0 {
		return nil, fmt.Errorf("the TDNS agent does not support primary zones: %v", primary_zones)
	}
