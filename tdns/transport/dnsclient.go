...
func NewDNSClient(transport core.Transport, port string, tlsConfig *tls.Config) *DNSClient {
	if tlsConfig == nil {
		switch transport {
		case core.TransportDoT, core.TransportDoH:
			tlsConfig = &tls.Config{
...
func (c *DNSClient) Exchange(msg *dns.Msg, server string) (*dns.Msg, time.Duration, error) {
	...
	if Globals.Debug {
		fmt.Printf("*** Exchange: sending %s message to %s:%s opcode: %s qname: %s rrtype: %s\n",
			core.TransportToString[c.Transport], server, c.Port,
