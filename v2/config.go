...
type Config struct {
	...
	// ChildSync and ParentSync are the childsync: and parentsync: blocks, named
	// for the zone options that switch them on. Partial by design — see the type
	// comments in config_delegationsync.go. No validate tags and not registered
	// in ValidateConfig's configsections, so a config without them is exactly as
	// valid as it was before these fields existed.
	...
	// DeprecatedDelegationSync is the retired `delegationsync:` wrapper, still
	// accepted for a deprecation cycle. FoldDeprecatedDelegationSync moves it
	// onto the two fields above and clears it; nothing else reads it.
	DeprecatedDelegationSync *DeprecatedDelegationSyncConf `yaml:"delegationsync" mapstructure:"delegationsync"`
