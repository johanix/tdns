...
func TestClassifyUnusedConfigKeys(t *testing.T) {
	unused := []string{
		...
		// The same retired keys under the block's new home: an operator who
		// moved delegationsync.parent: to the top-level childsync: but kept a
		// retired subkey must still be told it is retired, not that it is a typo.
		"childsync.update.key-verification",
		"childsync.bootstrap",
