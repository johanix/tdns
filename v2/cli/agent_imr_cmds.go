...
func newImrResetCmd(role string) *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			amr, err := SendImrMgmtCmd(cmd.Context(), role, &tdns.ImrMgmtPost{Command: "imr-reset"})
			if err != nil {
				log.Fatalf("Request failed: %v", err)
			}
			if amr.Error {
...
func newImrDumpTuningCmd(role string) *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			amr, err := SendImrMgmtCmd(cmd.Context(), role, &tdns.ImrMgmtPost{Command: "imr-dump-tuning"})
			if err != nil {
				log.Fatalf("Request failed: %v", err)
			}
			if amr.Error {
...
func imrMgmtOrDie(ctx context.Context, role string, post *tdns.ImrMgmtPost) *tdns.ImrMgmtResponse {
	amr, err := SendImrMgmtCmd(ctx, role, post)
	if err != nil {
...
func newImrForwardCmd(role string) *cobra.Command {
	...
	c.AddCommand(
		&cobra.Command{
			...
			Args:  cobra.NoArgs,
			Run: func(cmd *cobra.Command, args []string) {
				amr := imrMgmtOrDie(cmd.Context(), role, &tdns.ImrMgmtPost{Command: "imr-forward-list"})
		...
		&cobra.Command{
			...
			Args:  cobra.NoArgs,
			Run: func(cmd *cobra.Command, args []string) {
				amr := imrMgmtOrDie(cmd.Context(), role, &tdns.ImrMgmtPost{Command: "imr-forward-status"})
...
error and a newly dead one raises it.`,
			...
			Run: func(cmd *cobra.Command, args []string) {
				...
				amr := imrMgmtOrDie(cmd.Context(), role, post)
...
func newImrStubCmd(role string) *cobra.Command {
	...
	c.AddCommand(
		&cobra.Command{
			...
			Args:  cobra.NoArgs,
			Run: func(cmd *cobra.Command, args []string) {
				amr := imrMgmtOrDie(cmd.Context(), role, &tdns.ImrMgmtPost{Command: "imr-stub-list"})
		...
		&cobra.Command{
			...
			Args:  cobra.NoArgs,
			Run: func(cmd *cobra.Command, args []string) {
				amr := imrMgmtOrDie(cmd.Context(), role, &tdns.ImrMgmtPost{Command: "imr-stub-status"})
...
// NewImrSubtree builds the `imr` cache-inspection subtree for one daemon
// instance.
//
// The leaves were already role-parameterised (addImrLeafCmds threads role into
// each, and they pass it to SendImrMgmtCmd); only the two PARENTS were
// package-level vars, one per daemon, which is what kept the subtree off any
// further instance. A *cobra.Command has one parent, so a per-instance subtree
// needs a per-instance root.
func NewImrSubtree(role string) *cobra.Command {
	c := &cobra.Command{
		Use:   "imr",
		Short: "IMR (Internal Recursive Resolver) cache commands",
	}
	TagRole(c, role)
	addImrLeafCmds(c, role)
	return c
}

func init() {
	AgentCmd.AddCommand(NewImrSubtree("agent"))
	AuthCmd.AddCommand(NewImrSubtree("auth"))
