		...
		for i := 0; i < val.NumField(); i++ {
			...
			if fieldValue == nil {
				...
				func() {
					...
					defer func() {
						if r := recover(); r != nil {
							lg.Warn("deepCopyAndSanitize: struct field type", "type", reflect.TypeOf(v).String())
							lg.Warn("deepCopyAndSanitize: failed to set field", "index", i, "field", val.Type().Field(i).Name, "error", r)
		...
		for i := 0; i < val.Len(); i++ {
			...
			if elemValue != nil {
				func() {
					defer func() {
						if r := recover(); r != nil {
							lg.Warn("deepCopyAndSanitize: failed to set slice element", "index", i, "error", r)
						}
					}()
		...
		for i := 0; i < val.Len(); i++ {
			...
			if elemValue != nil {
				func() {
					defer func() {
						if r := recover(); r != nil {
							lg.Warn("deepCopyAndSanitize: failed to set array element", "index", i, "error", r)
						}
					}()
					copy.Index(i).Set(reflect.ValueOf(elemValue))
				}()
		...
		for _, key := range val.MapKeys() {
			...
			if elemValue != nil {
				func() {
					defer func() {
						if r := recover(); r != nil {
							lg.Warn("deepCopyAndSanitize: failed to set map key", "key", key.Interface(), "error", r)
						}
					}()
...
func SanitizeForJSON(v interface{}) interface{} {
	...
	func() {
		defer func() {
			if r := recover(); r != nil {
				lg.Warn("sanitization failed", "error", r)
