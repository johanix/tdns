		...
		if !allowInsecure {
			return nil, fmt.Errorf(
				"the published endpoint %q is not https; refusing to send a bearer credential in clear"+
					" (set parentsync.api.allow-insecure to override, in a lab only)", endpoint.Url)
		}
...
func dsyncApiHttpClient(caFile string, cred DsyncApiClientCredential, addrs []string) (*http.Client, error) {
	...
	if strings.TrimSpace(caFile) != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("reading parentsync.api.cafile %q: %v", caFile, err)
		}
		...
		if err != nil || pool == nil {
			lgDsyncApi.Warn("system certificate pool unavailable;"+
				" verifying against parentsync.api.cafile alone."+
