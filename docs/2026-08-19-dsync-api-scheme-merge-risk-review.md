...
- `viper.GetStringSlice("delegationsync.childsync.schemes")` → `[]`, so
  `PublishDsyncRRs` iterates an empty list and **publishes nothing**;
- `viper.GetString("delegationsync.childsync.update.target")` → `""`, so
...
| New DSYNC API listener | `SetupDsyncApiRouter` returns nil unless `api` ∈ `delegationsync.childsync.schemes`; `StartDsyncApiListener` returns nil on a nil router. **No socket bound unless opted in**, no default listen address. Refuses to start without TLS rather than falling back to plaintext. |
...
- Finish the viper → struct migration for the `delegationsync.parentsync.*` and
