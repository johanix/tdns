...
2. **The plan's one concrete D-6 config instruction targets the wrong side of the tree.** It says to add `childsync.bootstrap.methods` — that field already exists (`v2/config_delegationsync.go:54-56`, `DelegationSyncParentConf.Bootstrap.Methods`, added by `delegationsync-config-struct` #360). D-6 is a *child*-side behavior; `DelegationSyncChildConf` (`:59-64`) has nowhere to put a bootstrap-method preference. #360 also imposes a "single reader, no viper" contract on this struct that the plan predates.
