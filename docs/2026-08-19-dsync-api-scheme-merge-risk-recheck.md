...
`api` in `childsync.schemes` is therefore **inert today**.
...
| Child `parentsync.*` / keygen / key-verification | Still viper. Viper is populated. The "half-done migration" is real as a consistency footnote, not a behaviour change. |
...
1. Live `delegationsync:` uses list form for `schemes` / `types` / `addresses`
   ...
   newly fail.
2. Live `childsync.schemes` does **not** already contain `api`
