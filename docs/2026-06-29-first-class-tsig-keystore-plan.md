...
|---|---|---|---|---|
| 1 | `TsigKeystore` table in `db_schema.go` + creation; reserved-name/`validateTsigKeySpec` on DB insert | Low | ~50 | in progress |
| 2 | DB CRUD `TsigKeyMgmt` + cache-after-commit discipline (§4); in-memory store loads from DB | Med | ~220 | pending |
| 3 | Boot reorder (§5): KeyDB before `LoadTsigKeys`; `LoadTsigKeys` = load DB + sync `keys.tsig` (`main_initfuncs.go:123,130`) | Med | ~120 | pending |
| 4 | Reload reconcile in place, three-mode/no-silent-overwrite (§6); **`config reload-tsig`** CLI + API; **keep** legacy YAML re-merge on reload until step 12 | Med | ~140 | pending |
| 5 | Reference-count scan over `Zones` + catalog groups (§8) | Low | ~70 | pending |
