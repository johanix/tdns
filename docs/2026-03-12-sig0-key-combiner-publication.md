...
After winning a leader election, `onLeaderElected` (main_initfuncs.go) should generate a SIG(0) keypair and publish the KEY RR. Currently it bails out because it checks `delegationsync.parentsync.schemes` for `"update"` — but that config controls *direct parent UPDATE*, not combiner-mediated publication.
...
The old guard (`delegationsync.parentsync.schemes` config) controlled direct parent UPDATE and is not relevant for combiner-mediated publication.
