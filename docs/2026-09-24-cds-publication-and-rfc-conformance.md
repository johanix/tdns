...
- **I7** compares the served CDNSKEY with the served CDS whatever the key rows say, and reports a CDNSKEY served with no CDS: tdns never publishes one alone. The comparison with the rows' `ds` column is unchanged.
- **A keystore that cannot be read** makes the publish fail rather than leave the CDNSKEY out. For a key that exists only in the keystore, leaving it out would drop a good CDNSKEY and bring it back on the next good read.
