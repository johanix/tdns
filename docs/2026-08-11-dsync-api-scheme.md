...
requests a client certificate only when `childsync.api.client-auth`
...
- **TLS is mandatory.** A URI with an `http://` scheme is refused by the child
  ...
  reads like what it is (`parentsync.api.allow-insecure`), because
...
`childsync.*` block is read with `viper.GetString` /
...
- A parent that does not configure `childsync.api` publishes no API
...
protection at all. One switch, `parentsync.api.allow-insecure`,
...
- **TLS with the internal CA works.** `tdns-cli cert ca` + `cert leaf` for
  ...
  This is what `parentsync.api.cafile` exists for — trusting a
