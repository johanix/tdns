...
- **Current (PARTIAL — reassessed 2026-09-02 after PR #471):**
  - **(ii) DONE:** the receiver's KEY is published at the DSYNC {target} — `SetupZoneSync` now calls `DsyncUpdateTargetName` (the same helper DSYNC publication and the responder use), so a root-zone parent no longer computes a different target for key publication than for DSYNC publication/signing.
...
- **Current (PARTIAL):** DS/RFC7344/8078 done via `CheckDelegationCoherence` (see above); the RFC7477 NS/glue checks (`CsyncAnalyzeNS:373`, `CsyncAnalyzeA:259`, `CsyncAnalyzeAAAA:317` in `v2/scanner_csync.go`, called from `ProcessCSYNCNotify` in `scanner.go`) remain scanner-only and are not reused for UPDATEs; the UPDATE path's own gate is otherwise just SIG(0)-trust + name-scope + RR-type policy (`v2/updateresponder.go:592-618`).
